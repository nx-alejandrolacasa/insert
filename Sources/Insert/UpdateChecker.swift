...
private let log = Logger(subsystem: "com.alejandrolacasa.insert", category: "UpdateChecker")
...
/// use by hand. Ported from prtscn's checker of the same name.
///
...
final class UpdateChecker {
    ...
    private static let latestReleaseAPI =
        URL(string: "https://api.github.com/repos/nx-alejandrolacasa/insert/releases/latest")!
    ...
    struct Release: Equatable {
        let version: String   // "0.15.0" — tag with the leading "v" stripped
    ...
    /// the release page instead — replacing /Applications/Insert.app from a
    /// dev build would clobber an app we're not running.
    func installLatest() async {
        guard let release = latest else { return }
        guard let dmgURL = release.dmgURL, !BuildVariant.isDev else {
    ...
    private static func install(dmg: URL) async throws {
        let mountPoint = FileManager.default.temporaryDirectory
            .appendingPathComponent("insert-update-\(UUID().uuidString)")
    ...
    /// the main actor suspends instead of blocking.
