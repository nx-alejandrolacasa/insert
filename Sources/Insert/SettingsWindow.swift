...
/// The Settings window, built the way System Settings builds its own: an AppKit
/// `NSSplitViewController` whose sidebar item supplies the native full-height
/// sidebar — its material runs from the very top of the window (traffic lights
/// float over it) to the bottom. SwiftUI's `NavigationSplitView` can't produce
/// this on macOS 26: it renders the sidebar as a floating panel inset below the
...
final class SettingsWindowController: NSObject, NSWindowDelegate {
    ...
    func show() {
        ...
        // carries the pane header: a tracking separator keeps the header on the
...
extension SettingsWindowController: NSToolbarDelegate {
    ...
    func toolbar(
        ...
        // Otherwise AppKit wraps the whole item — chevrons *and* pane title — in
        // one glass capsule. The chevrons bring their own; the title wants none.
